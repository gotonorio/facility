...
def get_repair_plan_summary(version, kouji_type="ALL"):
    """長期修繕計画の一覧と合計を取得"""
    queryset = KoujiName.get_koujiname_list(version, kouji_type).order_by("kouji_year", "kouji_type")
    return queryset, calculate_repair_total(queryset)


def get_yearly_repair_summary(version, year):
    """特定年度の修繕計画と合計を取得"""
    queryset = KoujiName.get_repair_plan_list_by_year(version, year).order_by("kouji_type", "kouji_name")
    return queryset, calculate_repair_total(queryset)
...
def get_koujitype_aggregation(version):
    ...
    qs = (
        KoujiName.get_koujiname_list(version, "ALL")
