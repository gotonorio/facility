...
class MotorCycleSpaceUpdateView(PermissionRequiredMixin, UpdateView):
    ...
    def get_success_url(self):
        ...
        return reverse("motorcycle:list", kwargs={"year": year, "month": month})
    ...
    def form_valid(self, form):
        ...
        room = int(form.cleaned_data["room_no"])
        ...
        if room == 0 and status != "空き":
            ...
            return redirect("motorcycle:update", self.kwargs["pk"])
...
class MonthlyProcessingView(PermissionRequiredMixin, FormView):
    ...
    def latest_data(self):
        """バイク置場データから使用中の最新日付を返す"""
        qs = MotorCycleSpace.objects.values("date").filter(status_of_use="使用中")
    ...
    def get_context_data(self, **kwargs):
        ...
        # 登録済みデータ
        qs = MotorCycleSpace.objects.values("date")
    ...
    def post(self, request, *args, **kwargs):
        ...
        qs = MotorCycleSpace.objects.all().filter(date=new_date)
        if len(qs) < 1:
            # 最新の駐車場データ
            ...
            MotorCycleSpace.objects.bulk_create(new_parking)
        else:
            messages.info(self.request, f"バイク置場の{new_date}は既に存在しています。")
        return redirect(self.get_success_url())
